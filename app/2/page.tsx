...
export default function DesignTwo() {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Blue Sidebar */}
      <div className="w-64 bg-gradient-to-b from-blue-800 to-blue-900 text-white flex flex-col p-6">
        <h1 className="text-xl font-bold mb-8">Trading System</h1>
        <nav className="space-y-3 text-sm flex-1">
          <div className="px-4 py-2 bg-blue-700 rounded">📊 Dashboard</div>
          <div className="px-4 py-2 hover:bg-blue-700 rounded cursor-pointer">📈 Positions</div>
          <div className="px-4 py-2 hover:bg-blue-700 rounded cursor-pointer">⚡ Signals</div>
          <div className="px-4 py-2 hover:bg-blue-700 rounded cursor-pointer">📋 History</div>
          <div className="px-4 py-2 hover:bg-blue-700 rounded cursor-pointer">⚙️ Settings</div>
        </nav>
        <button onClick={toggleMonitor} className={`w-full py-2 rounded font-semibold ${monitorStatus === 'running' ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`}>
      ...
      <div className="flex-1 overflow-auto">
        <div className="sticky top-0 bg-white border-b border-gray-300 px-8 py-4 z-50">
          <h2 className="text-2xl font-bold text-gray-900">DESIGN 2: Corporate Blue Sidebar</h2>
          <p className="text-sm text-gray-600 mt-1">Last update: {lastUpdate} ADL</p>
        </div>
        ...
        {account && (
          <div className="px-8 py-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: 'Balance', value: `$${account.balance.toLocaleString('en-US', {minimumFractionDigits: 2})}`, icon: '💰', color: 'from-blue-500 to-blue-600' },
                { label: 'Equity', value: `$${account.equity.toLocaleString('en-US', {minimumFractionDigits: 2})}`, icon: '📊', color: 'from-indigo-500 to-indigo-600' },
                { label: 'P&L', value: `${account.unrealizedPnL >= 0 ? '+' : ''}$${account.unrealizedPnL.toLocaleString('en-US', {minimumFractionDigits: 2})}`, icon: '📈', color: account.unrealizedPnL >= 0 ? 'from-green-500 to-green-600' : 'from-red-500 to-red-600' },
                { label: 'Margin', value: `${account.marginBuffer.toFixed(1)}%`, icon: '🛡️', color: 'from-purple-500 to-purple-600' },
              ].map((m, i) => (
                <div key={i} className={`bg-gradient-to-br ${m.color} rounded-lg p-6 text-white shadow-md`}>
        ...
        <div className="px-8 py-6 border-t border-gray-200">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Market Data</h3>
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-100 border-b border-gray-200">
                <tr>
                  <th className="px-6 py-4 text-left font-semibold text-gray-700">Instrument</th>
                  <th className="px-6 py-4 text-right font-semibold text-gray-700">Price</th>
                  <th className="px-6 py-4 text-right font-semibold text-gray-700">RSI</th>
              ...
              <tbody>
                {[
                  ...
                  <tr key={inst.symbol} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-right">{inst.price.toLocaleString()}</td>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            {positions.length === 0 ? (
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-100 border-b border-gray-200">
                  <tr>
                    <th className="px-6 py-4 text-left font-semibold text-gray-700">Symbol</th>
                    <th className="px-6 py-4 text-left font-semibold text-gray-700">Direction</th>
                    <th className="px-6 py-4 text-right font-semibold text-gray-700">Entry</th>
                    <th className="px-6 py-4 text-right font-semibold text-gray-700">Current</th>
                    <th className="px-6 py-4 text-right font-semibold text-gray-700">P&L</th>
                  </tr>
                </thead>
                <tbody>
                  {positions.map((pos) => (
                    <tr key={pos.symbol} className="border-b border-gray-200 hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-right font-semibold text-green-600">+${pos.pnl.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {/* Pending Trades */}
        <div className="px-8 py-6 border-t border-gray-200 pb-12">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Pending Approval <span className="bg-red-600 text-white px-2 py-1 rounded text-sm">{pendingTrades.length}</span></h3>
          <div className="bg-white rounded-lg shadow overflow-hidden">
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-100 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left font-semibold text-gray-700">Direction</th>
                    <th className="px-6 py-4 text-right font-semibold text-gray-700">Entry</th>
                    <th className="px-6 py-4 text-right font-semibold text-gray-700">Risk</th>
